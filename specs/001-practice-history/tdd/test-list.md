...
scenario of User Story 2). The 12 scenarios are the `spec_criteria`. Requirements are `FR-001` to
`FR-015`, success criteria `SC-001` to `SC-006`, and `Edge: <topic>` names a bullet under Edge Cases.
...
| A7  | Loading an entry moves it to the top of the list and raises its practice count by one, without adding a second entry                   | US2.3, FR-009              | example          | PENDING |      |
...
| A10 | Typing a text to its last character raises that entry's completed count by exactly one, visible without a reload                       | US3.2, FR-010, SC-004      | example          | PENDING |      |
| A11 | Pressing Reset, or loading another entry, before finishing leaves the completed count unchanged while the practice count reflects it   | US3.3, FR-010              | example          | PENDING |      |
...
| A13 | On first visit with no history the sidebar shows the empty-state message                                                               | FR-011, Edge: empty        | example          | PENDING |      |
| A14 | When saving to the store fails, practice still starts and the sidebar announces that history could not be saved                        | FR-013, Edge: storage      | example          | PENDING |      |
| A15 | When reading the store fails, the sidebar announces that history could not be loaded and practice can still be started                | FR-013, Edge: storage      | example          | PENDING |      |
| A16 | Data from the previous app version, including duplicate rows of one text, appears as one entry per text with summed counts             | FR-012, Edge: legacy data  | example          | PENDING |      |
| A17 | Starting with a text and again with the same text plus trailing spaces or newlines leaves one entry                                    | FR-002, Edge: trailing ws  | example          | PENDING |      |
| A18 | Activating Load on the same entry several times in a row leaves one entry per text and one running session on the loaded text         | Edge: repeated Load        | example          | PENDING |      |
...
| U45 | Shows "Practiced 1 time" for a practice count of 1                                                    | FR-006         | example          | PENDING |      |
| U46 | Shows "Practiced 2 times" for a practice count of 2                                                   | FR-006         | example          | PENDING |      |
| U47 | Shows "Completed 0 times" for a completion count of 0                                                 | FR-006         | example          | PENDING |      |
| U48 | Shows "Completed 1 time" for a completion count of 1                                                  | FR-006         | example          | PENDING |      |
| U49 | Shows "Completed 2 times" for a completion count of 2                                                 | FR-006         | example          | PENDING |      |
...
| U14 | recordPractice on a new text creates exactly one entry: practice count 1, completed count 0, dateCreated equal to dateModified | FR-001                | example | PENDING |      |
| U15 | recordPractice on an existing text keeps one entry: practice count +1, dateModified later, dateCreated and completed unchanged | FR-002, FR-009        | example | PENDING |      |
...
| U17 | recordPractice with a text differing only by case or inner spacing creates a separate entry                                    | FR-002                | example | PENDING |      |
| U18 | recordPractice with "" or whitespace-only writes nothing and resolves ok                                                       | FR-003                | example | PENDING |      |
| U19 | Two recordPractice calls for the same text issued together leave one entry with practice count 2                               | Edge: repeated Load   | example | PENDING |      |
| U20 | When the store rejects the write, recordPractice resolves { ok: false } and does not reject                                    | FR-013                | example | PENDING |      |
| U21 | recordCompletion raises only the completed count, by one, leaving practice count and both dates unchanged                      | FR-010                | example | PENDING |      |
| U22 | Two recordCompletion calls for one text give a completed count of 2                                                            | FR-010                | example | PENDING |      |
...
| U25 | useHistory reports { status: "loading" } first, then { status: "ready" }                                                       | FR-005                | example | PENDING |      |
| U26 | useHistory on an empty store is ready with an empty entries array                                                              | FR-011                | example | PENDING |      |
| U27 | useHistory returns entries in descending dateModified order                                                                   | FR-005                | example | PENDING |      |
| U28 | useHistory returns every entry: all 6 when 6 exist (past the old cap of 5), and all 100 when 100 exist                         | FR-005, SC-006        | example | PENDING |      |
| U29 | useHistory updates without remounting after recordPractice and after recordCompletion                                          | SC-004                | example | PENDING |      |
| U30 | When the store cannot be read, useHistory returns { status: "error" } and does not throw during render                        | FR-013                | example | PENDING |      |
...
| U34 | The merged row's practice count and completed count are the sums of the group                                               | FR-012           | example          | PENDING |      |
...
- **Repeated Load and the practice count.** The spec says repeated Load leaves no duplicate entries; it
  does not say whether each activation raises the practice count. A18 asserts entries and session only.
  If the count should rise only once per real session, that is a spec change.
- **Setup text box after a Load.** After loading an entry and pressing Reset, does the setup box keep
  the earlier draft, show the loaded text, or clear? The plan is silent and the behavior is not
  specified, so no test pins it.
...
- **Concurrent writes from two browser tabs.** The unique index is the backstop (U38). What
  recordPractice does on a ConstraintError (resolve `{ ok: false }` or retry as an update) is not
  decided in the plan or contracts. fake-indexeddb in one test process cannot easily model two tabs.
- **Completion recorded more often than practice.** `recordCompletion` does not check the counts, so
  `numberOfCompletes <= numberOfLoads` holds only because App records a completion only from a running
  session. No reachable UI path double-counts, so it is guarded by A10 (exactly one) and not a
  separate test.
