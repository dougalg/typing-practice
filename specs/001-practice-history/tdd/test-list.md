...
| A16 | Data from the previous app version, including duplicate rows of one text, appears as one entry per text                                 | FR-012, Edge: legacy data  | example          | PENDING |      |
...
| U19 | Two recordPractice calls for the same text issued together leave exactly one entry                                              | Edge: repeated Load    | example | PENDING |      |
| U20 | When the store rejects the write, recordPractice resolves { ok: false } and does not reject                                     | FR-013                 | example | PENDING |      |
| U68 | When inserting a new text fails with a unique-text ConstraintError (another writer created it first), recordPractice retries once as an update, leaves one entry and resolves ok | FR-017 | example | PENDING |      |
| U69 | When that retry also fails, recordPractice resolves { ok: false } and does not reject                                           | FR-013, FR-017         | example | PENDING |      |
