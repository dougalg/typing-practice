...
| A5  | Pressing Load on an entry opens the practice view with that entry's full text, typing input focused, 0 characters typed, in one click  | US2.1, FR-007, SC-001      | example          | DONE | `src/App.test.tsx::[A5]` |
| A6  | Loading a different entry while another text is half typed replaces it with a fresh attempt at the first character                      | US2.2, FR-008              | example          | DONE | `src/App.test.tsx::[A6]` |
| A7  | Loading an entry moves it to the top of the list and updates its last-practiced date, leaves its practice count unchanged and adds no second entry | US2.3, FR-009      | example          | DONE | `src/App.test.tsx::[A7]` |
| A8  | With the keyboard alone, Tab reaches an entry's Load button and Enter, then Space on another entry, each loads it                       | US2.4, FR-014, SC-005      | example          | DONE | `src/App.test.tsx::[A8]` |
| A9  | An entry with a very long text shows only a short preview, the other entries remain listed, and Load types the full text                | US3.1, Edge: long text     | example          | DONE | `src/App.test.tsx::[A9]` |
| A10 | Typing a text to its last character, even after a mistake on the way, raises that entry's practice count by exactly one, visible without a reload | US3.2, FR-010, SC-004 | example        | DONE | `src/App.test.tsx::[A10]` |
| A11 | Pressing Reset, or loading another entry, before finishing leaves the practice count unchanged                                          | US3.3, FR-010              | example          | DONE | `src/App.test.tsx::[A11]` |
| A12 | With many entries (100) the sidebar lists all of them, most recently practiced first                                                    | US3.4, FR-005, SC-006      | example          | DONE | `src/App.test.tsx::[A12]` |
...
| A19 | Starting, loading and finishing a practice make no network request (fetch, XHR, beacon are never called): a guard, cannot start red    | FR-015                     | example          | DONE | `src/App.test.tsx::[A19]` |
...
| U21 | recordCompletion raises the practice count by one and leaves both dates unchanged                                               | FR-010                 | example | DONE | `src/features/savedItems/history.test.ts::[U21]` |
| U22 | Two recordCompletion calls for one text give a practice count of 2                                                              | FR-010                 | example | DONE | `src/features/savedItems/history.test.ts::[U22]` |
| U23 | recordCompletion for a text with no entry resolves ok and creates no entry                                                      | FR-010                 | example | DONE | `src/features/savedItems/history.test.ts::[U23]` |
| U24 | When the store rejects the write, recordCompletion resolves { ok: false } and does not reject                                   | FR-013                 | example | DONE | `src/features/savedItems/history.test.ts::[U24]` |
...
| U70 | useHistory updates without remounting after recordCompletion (split from U29; drive once recordCompletion exists, task T021)     | SC-004                 | example | DONE | `src/features/savedItems/history.test.ts::[U70]` |
